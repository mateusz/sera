...
type Config struct {
	...
	FirstPastThePost bool
}

func init() {
	flag.BoolVar(&conf.FirstPastThePost, "first-past-the-post", false, "First past the post will only execute the command if the lock was free at first attempt.")
	flag.Parse()
}
...
func realMain() (int, error) {
	...
	if conf.FirstPastThePost {
		return firstPastThePost(conn, keyName, timeout)
	} else {
...
func firstPastThePost(c *MysqlConnection, keyName string, timeout time.Duration) (int, error) {
